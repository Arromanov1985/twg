...
def render_visual_kp(selected_df: pd.DataFrame, reasons: list[str], values: dict[str, Any], client_data: dict[str, Any]) -> None:
    ...
    st.subheader("Рекомендации по обслуживанию коттеджной системы")

    service_recommendations = [
        "Проверять наличие таблетированной соли в солевом баке не реже 1 раза в 2–4 недели.",
        "Не допускать полного опустошения солевого бака — уровень соли должен быть выше уровня воды.",
        "Заменять картриджи механической очистки по мере загрязнения, обычно 1 раз в 1–3 месяца.",
        "Проводить сервисное обслуживание клапанов управления и фильтрующих колонн не реже 1 раза в год.",
        "Контролировать давление воды до и после системы, чтобы вовремя выявлять засорение фильтров.",
        "После длительного простоя системы выполнить промывку и при необходимости санитарную обработку.",
    ]
...
SYSTEM_SCHEME_RULES = [
    {
        ...
        "image": "df100.png",
        "keywords": ["twg df-034", "twg df-100", "df-034", "df-100", "sdf", "fd-100", "дисков"],
        "description": "Дисковый фильтр устанавливается на входе системы и задерживает песок, ржавчину, окалину и другие крупные механические примеси. Он защищает клапаны управления, колонны и последующие ступени очистки от преждевременного загрязнения.",
    },
    {
        "title": "Аэрация",
        ...
        "description": "Аэрационная колонна насыщает воду воздухом. Это помогает окислить растворённое железо, марганец и сероводород, чтобы затем удалить их на фильтрующей загрузке. Также аэрация снижает неприятный запах воды.",
    },
    {
        ...
        "description": "Колонна с фильтрующей загрузкой удаляет из воды окисленное железо, марганец, мутность и взвешенные загрязнения. Эта ступень отвечает за основную очистку воды после предварительного окисления.",
    },
    {
        "title": "Умягчение",
        ...
        "description": "Умягчитель снижает жёсткость воды методом ионного обмена. Это защищает котёл, бойлер, сантехнику, смесители и бытовую технику от накипи. Солевой бак используется для автоматической регенерации загрузки.",
    },
    {
        "title": "BB20",
        ...
        "description": "Корпус BB20 используется для финишной картриджной очистки. Он задерживает мелкие механические частицы после основных колонн и дополнительно защищает сантехнику, бытовую технику и питьевую систему.",
    },
    {
        "title": "OSMOS",
        ...
        "description": "Система обратного осмоса устанавливается для получения питьевой воды на кухне. Она снижает минерализацию, удаляет мелкодисперсные примеси и улучшает вкус воды для питья и приготовления пищи.",
    },
]
...
def build_system_scheme(water_data: dict, equipment_data: list[dict]) -> list[dict]:
    ...
    Картинки и описания берутся из SYSTEM_SCHEME_RULES.
    """
    ...
    def add(title: str, note: str, image: str = "", description: str = "", is_service: bool = False):
    ...
    add(
        ...
        "источник воды",
        "",
        "Исходная вода поступает из скважины или другого источника водоснабжения на вход системы очистки.",
        True,
    )
    ...
    add(
        "Дом",
        "чистая вода для потребителей",
        "",
        "После прохождения всех ступеней очистки вода подаётся к точкам водоразбора в доме.",
        True,
    )
